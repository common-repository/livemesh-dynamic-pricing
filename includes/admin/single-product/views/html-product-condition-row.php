...
?><div class='lwc-condition-wrap' data-index='<?php echo esc_attr( $wp_condition->index ); ?>'>
	...
	<span class='lwc-condition-field-wrap'><?php
		...
		$condition_field_args = array(
			'type'        => 'select',
			'name'        => 'dynamic_pricing[' . esc_attr( $wp_condition->index ) . '][condition][' . absint( $wp_condition->group ) . '][' . absint( $wp_condition->id ) . '][condition]',
	...
	<span class='lwc-operator-field-wrap'><?php
		...
		$operator_field_args = array(
			'type'    => 'select',
			'name'    => 'dynamic_pricing[' . esc_attr( $wp_condition->index ) . '][condition][' . absint( $wp_condition->group ) . '][' . absint( $wp_condition->id ) . '][operator]',
	...
	<span class='lwc-value-field-wrap'><?php
		$value_field_args = wp_parse_args( array(
			'value' => $wp_condition->value,
			'name'  => 'dynamic_pricing[' . esc_attr( $wp_condition->index ) . '][condition][' . absint( $wp_condition->group ) . '][' . absint( $wp_condition->id ) . '][value]',
		), $wp_condition->get_value_field_args() );
